...
- **2026-06-24** — **Fix: dashboard GM não rolava (Evento Telão) → migrado pra SCROLL NATIVO** (gm/index.html). O shell vivia com altura travada (`body` 100vh + `overflow-hidden`) e o scroll dependia de uma cadeia de flex aninhada (body→wrapper→main→div de conteúdo `flex-1 overflow-y-auto`) esticar perfeita. Tentativas com `min-h-0` e `max-height: calc(100dvh-4rem)` no container interno **não resolveram** no navegador do Kevin (a aba Evento Telão, a mais alta, simplesmente não rolava). Solução definitiva: **abandonar o scroll interno e usar o scroll nativo do documento**. Mudanças: `body` agora `min-height:100vh/100dvh` (cresce com o conteúdo) e **sem** `overflow-hidden`; wrapper principal sem `overflow-hidden` + `items-start`; **sidebar fixada via `md:sticky md:top-0 md:h-screen md:self-start`**; `<main>` sem `overflow-hidden` (+`min-w-0`); header `sticky top-0 z-20` (fica visível ao rolar, bg mais opaco); div de conteúdo virou só `p-4 md:p-8` (sem `flex-1/overflow/max-height`). Scroll nativo do body não tem como falhar. Sem bump (HTML servido direto pela Vercel).
