...
Versões atuais (índice rápido — confira no index.html antes de bumpar): `config v=31`, `auth v=32`, `api v=31`, `groups v=33`, `ui v=38`, `gm v=32`, `aovivo v=35`, `evento v=42`, `copa v=1`. (gm/index.html: `admin v=44`)
...
- [ ] **Copa Etapa 1.5 — Chaveamento PARCIAL / projeção** (ideia do Kevin). Antes da API ter os jogos do mata-mata, projetar os confrontos a partir da classificação atual seguindo o **template oficial do bracket 2026** (ex.: 1º do Grupo A × 2º do Grupo B). ⚠️ Complicação real: a Copa 2026 tem 48 times → 12 grupos → top 2 (24) + **8 melhores 3º colocados** (8) = 32 no Round of 32. A alocação dos 3ºs usa uma **tabela oficial** (15 combinações) — implementar isso é o trabalho. Versão simples primeiro: projetar 1º×2º conforme template; versão completa: incluir melhores 3ºs. Marcar tudo como "projeção (parcial)".
