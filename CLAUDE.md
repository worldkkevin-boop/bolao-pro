...
Versões atuais (índice rápido — confira no index.html antes de bumpar): `config v=31`, `auth v=32`, `api v=31`, `groups v=33`, `ui v=38`, `gm v=32`, `aovivo v=35`, `evento v=42`, `copa v=2`. (gm/index.html: `admin v=44`)
...
- [x] **Copa Etapa 1.5 — Chaveamento PARCIAL / projeção (Passo 1 FEITO)**. Projeção dos 16 jogos do Round of 32 da classificação atual via `COPA_R32_TEMPLATE` (cruzamentos oficiais por posição). Os jogos contra "melhor 3º" mostram o bolsão. **Falta (Passo 2):** resolver exatamente qual 3º vai pra cada jogo — tabela oficial **Annex C** (495 combinações pra 12 grupos escolhendo 8 melhores 3ºs, considerando ranking dos 3ºs por pontos/saldo/gols). E projetar fases seguintes (Oitavas→Final) é predição, não projeção — fica fora (vira Etapa 2/palpite).
