...
Versões atuais (índice rápido — confira no index.html antes de bumpar): `config v=31`, `auth v=32`, `api v=31`, `groups v=33`, `ui v=37`, `gm v=32`, `aovivo v=35`, `evento v=42`. (gm/index.html: `admin v=43`)
...
- **2026-06-19** — **Fix: dashboard GM não rolava até o fim no celular** (gm/index.html). O `<body>` era `h-screen` (100vh) + `overflow-hidden`, com o scroll no container interno (`flex-1 overflow-y-auto`). No mobile, `100vh` é maior que a área visível (conta a barra do navegador), então o fim do conteúdo ficava escondido atrás da barra e, como o body é `overflow-hidden`, não dava pra alcançar. Trocado `h-screen` → `h-[100dvh]` (altura dinâmica, respeita a barra). Sem bump (HTML servido direto).
