...
- **2026-06-24** — **Fix: dashboard GM (aba Evento Telão) não rolava até o fim** (gm/index.html). O container de conteúdo das abas (`<main>` → div de conteúdo) era `flex-1 overflow-y-auto` mas **sem `min-h-0`** — em flex-col o filho `flex-1` herda `min-height:auto`, então cresce até o tamanho do conteúdo em vez de respeitar a altura do `<main>`; o `overflow-y-auto` nunca disparava e o `<main>` (`overflow-hidden`) cortava o resto. Aparecia só no Evento Telão por ser a aba com mais conteúdo (única que estoura a tela). Fix: `min-h-0` no div de conteúdo (linha ~235). Sem bump (HTML servido direto pela Vercel).
